...
export const createDataCliets = (
  client: { clients: { discount: string }[] },
  data: { name: string; value: unknown }[]
) => {
  const discountArray: string[] = [];
  const counts: any = {};
  const dataArray = data;

  client.clients.forEach((item: { discount: string }) => {
    discountArray.push(item.discount);
  });

  discountArray.forEach((x) => {
  ...
  keys.forEach((_item: any, i: number) => {
...
export const exportToXlsx = (inputData: any, fileName: string) => {
  ...
  const excelBuffer = XLSX.write(dataTemplate, {
    bookType: "xlsx",
    type: "array",
  });
  const data = new Blob([excelBuffer], { type: fileType });
  const date = `_${new Date().getDate()}.${
    new Date().getMonth() + 1
